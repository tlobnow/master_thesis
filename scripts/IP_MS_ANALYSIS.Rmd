...
- retrieve protein accession numbers from gene names
...
``` {r}
TIME_CHECK %>% filter(Protein.IDs == "Q9Z321") %>%
...
SIGNIF_BY_iSCORE <- MAIN %>% filter(iScore >= 0.4, !is.na(Gene.Names)) %>% distinct(FILE, .keep_all = T)
...
To create figure XXX, the JSON files of each run were extracted, iScores were grouped by file and highest iScores are plotted. If multiple runs (different seeds) were performed per protein, the 
